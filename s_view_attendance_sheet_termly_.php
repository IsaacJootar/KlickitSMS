...
<p>
   ...
   <tr class="transcriptheader hdsmall">
    ...
    WHERE `class_name`='{$class_name}' AND `student_id`='{$s_id}'
     AND `term_id` = '{$term_id}'");
...
<td><div align="left"><?php echo  $students['fullname']; ?></div></td>
 <?php
    $sql=$database->query("SELECT  *  FROM `student_attendance` WHERE `class_name`='{$class_name}' AND `term_id` = '{$term_id}' AND `student_id`= '{$s_id}' GROUP BY `week` ");
while ($term_total=$database->fetch_array($sql)) { 
?>
 
  
 
<td><div align="left"><?php output_total($s_id, $term_id, $class_name, $term_total['week'])?></div> </td>

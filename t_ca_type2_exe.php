p<?php include('includes/t_header.php'); ?>
  ...
  $sql=mysql_query("SELECT  * FROM `score_entry` WHERE `subject_name`='{$_SESSION['subcat']}' AND `class_name`='{$_SESSION['cat']}' AND `staff_id`='{$id}'  AND `session_id`='{$sess_id}' AND `term_id` = '{$term_id}'");
		 ...
		 $query=mysql_query($query);
		  if(mysql_num_rows($query) < 1){
			$query="INSERT INTO `score_entry` 
				(`staff_id`, `class_name`, `subject_name`,`session_id`, `term_id`, `student_id`, `CA1`, `CA2`,`CA_total`) 
		VALUES ('{$id}', '$class', '$subj', '{$sess_id}', '{$term_id}' , '$qid', '$ca1', '$ca2','{$ca_total}') ";
 	...
 	for($i=0;$i<count($qst);$i++){
		...
		$query="INSERT INTO `score_entry` 
				...
				`student_id`, `CA1`, `CA2`,`CA_total`) 
		VALUES ('{$id}', '$class', '$subj', '{$sess_id}', '{$term_id}' , '$qid', '$ca1', '$ca2',
		'{$ca_total}') ";
			$result=mysql_query($query);

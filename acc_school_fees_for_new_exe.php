...
global $database;
	...
	$error_flag=false;
	// check sec_id 
	$check_id=$database->query("SELECT `sec_id` FROM `acc_configure_school_fees` WHERE `sec_id`= '{$sec_id}' AND `sess_id`='{$sess_id}' AND `term_id`= '{$term_id}' AND `status`= 0");
	...
	{	
		...
		session_write_close();
		redirect_to('acc_school_fees_for_new.php');
		exit();
	}
	...
	VALUES ('{$expected_to_pay}', '{$sec_id}', '{$sec_name}', '{$sess_id}', '{$term_id}', '0')");

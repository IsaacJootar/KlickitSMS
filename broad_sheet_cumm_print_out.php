...
$session=mysql_query("SELECT * FROM `session_manager` WHERE `id`='{$_SESSION['sess']}'");
  ...
  $term=$term['term_def'];
      ...
      ?>
                ...
                <h3 align="center" style="font-weight:bold">JETHRONE EAGLES ACADEMY, KUBWA, ABUJA. </h3>
        <h5 align="center" style="font-weight:bold">Address:Plot 168, Old Gbazango Layout, By Pipeline Extention, Kubwa, Abuja. 08032557312, 08053934853</h5>
 ...
 <h5 align="center"><a href="javascript:window.print()">[BroadSheet Print Out]</a></h5>
    ...
    while($sub=mysql_fetch_array($sql)){ ?> 
                            <th class="rotate" style="font-weight:1"><div>
                              <div align="center"><span><font size="-1"> <?php echo $sub['subject_name'];?></font></span></div>
                            </div> </th> 
...
-o-transform: rotate(-90deg);
  border-collapse:collapse;
      ...
      $sql2=mysql_query("SELECT distinct `score_entry`. `student_id`, `students`. `fullname` FROM `score_entry` JOIN `students` ON `score_entry`. `student_id`=`students`. `id` WHERE `class_name`= '{$_SESSION['class_name']}'
      AND `session_id`='{$_SESSION['sess']}' AND `account_status` = 1 ORDER BY trim(fullname) ASC");

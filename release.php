 ...
 <div align="center">
            ...
            <div class="box-inner">
                <div class="box-header well" data-original-title="">
                    <h2>Select parameters for releasing  result</h2>
...
self.location='release.php?cat=' + val ;
}
...
echo "</select>";
       ...
       $names = mysql_query("SELECT * FROM `student_class` WHERE  `class_name`= '{$_POST['class']}' AND `account_status` = 1 ORDER BY trim(fullname) ASC");
while($students=mysql_fetch_array($names)) {
$sta=mysql_fetch_array(mysql_query("SELECT `status` FROM `score_entry`
  ...
  if($sta['status']==1){
    ...
    ?>
               ...
               <td class="center">          
                   ...
                   <input type="submit" name="submit" value="release result"/>
                   </form>
                 </div>
               </div>
             </div>

...
$session=mysql_query("SELECT * FROM `session_manager` WHERE `id`='{$_SESSION['sess']}'");
	...
	$term=$term['term_def'];
        ...
        <h5 align="center" style="font-weight:bold">Address:<?php echo $school_address=schoolInformation::find_school_address();?> </h5>
         <h5 align="center" style="font-weight:bold" style="color:#000">BroadSheet Generated for <?php echo $_SESSION['class_name']; ?> </h5>
  ...
  border-collapse:collapse;
	  ...
	  WHERE `class_name`='{$_SESSION['class_name']}'");
	  		while($sub=mysql_fetch_array($sql3)){
				...
				$sub=$sub['subject_name'];
				 ...
				 ?>
                            ...
                            <?php 
            $sql4=mysql_query("SELECT * FROM `score_entry` WHERE `student_id`='{$student_id}' AND `class_name`='{$_SESSION['class_name']}' AND `subject_name` = '{$sub}' AND `session_id`='{$_SESSION['sess']}' AND `term_id`='{$_SESSION['term']}' AND `account_status` = 1");
        ...
        ?></font></div>
			...
			$query2=mysql_num_rows(mysql_query("SELECT `subject_name` FROM `score_entry` WHERE `student_id`='{$student_id}' AND `class_name`='{$_SESSION['class_name']}' AND `session_id`='{$_SESSION['sess']}' AND `term_id`='{$_SESSION['term']}'"));
		?>
...
$query4="SELECT  * FROM `score_entry_average` WHERE `student_id`='{$student_id}' AND `class_name`='{$_SESSION['class_name']}' AND `session_id`='{$_SESSION['sess']}' AND `term_id`='{$_SESSION['term']}'";
		...
		VALUES ('{$student_id}', '{$_SESSION['class_name']}', '{$_SESSION['cat']}', '{$_SESSION['subcat']}', '{$av}')" ;
			...
			$query5="UPDATE `score_entry_average` 
				SET
				 ...
				 `class_name`= '{$_SESSION['class_name']}',
				  `term_id`= '{$_SESSION['subcat']}',
				  `session_id` = '{$_SESSION['cat']}', 
				  `average`='{$av}'
				  WHERE `student_id`='{$student_id}' AND `class_name`='{$_SESSION['class_name']}' AND `session_id`='{$_SESSION['cat']}' AND `term_id`='{$_POST['subcat']}'
				   "; 

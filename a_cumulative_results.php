...
<?php
             ...
             if($page_count==1){$start=1; $end=10;}
              if($page_count==2){$start=11; $end=20;}
                if($page_count==3){$start=21; $end=30;}
             if($page_count==4){$start=31; $end=40;}
            if($page_count==5){$start=41; $end=50;}
          if($page_count==6){$start=51; $end=10;}
$rows=$database->query("SELECT `student_id` FROM `score_entry` WHERE `session_id`='{$_SESSION['sess']}'
             AND `class_name`='{$_SESSION['class_name']}' GROUP BY `student_id` LIMIT $end OFFSET $start ");
           if(!$rows){
...
<?php require_once('classes/result_manager_cumulative.php'); ?> 
        ...
        <p align="center">
    CUMULATIVE ACADEMIC REPORT (<?php echo $st=resultManager::find_current_session($_SESSION['sess']);  ?>)
</p>
 <p align="center"><a href="javascript:window.print()">[Print Out]</a></p>
  ...
  <tr class="transcriptheader hdsmall">
    ...
    <td width="155"><?php echo $pos=resultManagerCumulative::find_num_in_class($s_id, $student_class, $_SESSION['sess']); ?> <font size="-4"></td>
  ...
  <tr class="transcriptheader hdsmall">
   ...
   $class['class_name']; ?>
   <td><?php echo $cum_av=resultManagerCumulative::find_student_average($s_id,  $row['class_name'],  $_SESSION['sess']);  ?></td>
    <td>Result Type:</td>
    <td>Cumulative</td>
  </tr>
  <tr class="transcriptheader hdsmall">
    <td>Final Remark:</td>
    <td><?php  if($cum_av > 40){ echo "Passed";} else {echo "Failed";}?></td>
    <td>Class:</td>
    <td><?php 
      echo $student_class;
   ?></td>
  ...
  <tr class="transcriptheader hdsmall">
   ...
   $no=1;
   $all=resultManagerCumulative::find_student_all_by_id($s_id, $class['class_name'],  $_SESSION['sess']);

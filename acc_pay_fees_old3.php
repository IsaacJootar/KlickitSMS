...
// tuition config
$sql1=$database->query("SELECT * FROM  `acc_configure_tuition_fees` WHERE `sess_id`='{$sess_id}' AND  `term_id`= '{$term_id}' AND `status`=1  AND `class_name`='{$class_name}' ");
$expected_to_pay=$database->fetch_array($sql1); 
$tuition_exp=$expected_to_pay['expected_to_pay'];
 
$query=$database->query("SELECT `class_name`, `expected_to_pay`, `item_name`  FROM  `acc_configure_school_fees_items`  WHERE `sess_id`='{$sess_id}' AND  `term_id`= '{$term_id}' AND `status`=1  AND `class_name`='{$class_name}'");
    if (!$query){
 ...
 <div class="modal-header">
  ...
  <button type="button" class="close" data-dismiss="modal">X</button>
                ...
                <div class="modal-body">
                   <form class="form-horizontal" action="acc_pay_fees_old_exe.php" method="post">
...
echo  "<option value='Bank Deposit'>Bank Deposit</option>";
echo  "<option value='Cash at Hand'>Cash at Hand</option>";
echo  "<option value='Mobile / Phone transfer'>Mobile / Phone transfer</option>";
echo "</select>" . '</br>'. '</br>';

...
<p>
   ...
   <?php  
 global $database;
 $name=$database->query("SELECT *  FROM `student_class` WHERE  `class_name`='{$class_name}' order by fullname asc");
 ...
 $user=$database->query("SELECT *  FROM `students` WHERE `id`='{$exp['student_id']}'"); // use the student_id in student_Clas to get the details of student in students tables//
 if (!$name) { // add this check.
    echo('Invalid query: ' . mysql_error());
}

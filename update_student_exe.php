...
<?php
	...
	$student->firstname = ucwords(strtolower($_POST['firstname']));
	$student->fullname=  ucwords(strtolower($_POST['firstname']));
	$student->username =  str_replace(' ', '', $_POST['username']);
	...
	$qry=mysql_query("UPDATE `students`
		 ...
		 WHERE `id`= '{$_SESSION['id']}' ");
	$qry=mysql_query("UPDATE `system_roles`
		 SET `username`= '{$_POST['username']}',
		 `password`='{$_POST['username']}'
	...
	if($student->update()){
		...
		redirect_to('manage_student.php?class_name=' . $_SESSION['class_name']);
		exit();

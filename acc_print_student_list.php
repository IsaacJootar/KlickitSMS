...
?></h2>
        <p align="left">
        <h5 align="center" style="color:#600"><?php echo $school_address=schoolInformation::find_school_address();?> 
        </p>
        </h5></p>
...
<p>
    ...
    <td width="207"><div align="center"><strong>Registration No.</strong></div></td>
    <td width="207"><div align="center"><strong>Student's Name</strong></div></td>
    
  </tr>
   <?php  
 global $database;
 $name=$database->query("SELECT *  FROM `student_class` WHERE  `class_name`=$class_name");
 ...
 $user=$database->query("SELECT *  FROM `students` WHERE `id`='{$exp['student_id']}'");
 if (!$name) { // add this check.
    echo('Invalid query: ' . mysql_error());
}

...
// get the options, sent them in an array and check to make sure not more than one option is chosen as an answer//
$array = array("$oa", "$ob","$oc", "$od");
...
$sql=$database->query("INSERT INTO `cbt_questions`(`ques_text`, `tittle_text`, `score`, `staff_id`, `class`, `subject`) VALUES ('{$ques_text}', '{$tittle_text}', '{$score}', '$staff_id', '{$class}', '{$subject}')");	
	if ($sql){ 
		...
		header("location:cbt_tadd_questions2.php");
	exit();

 ...
 $pieChart->render();
  ...
  <body>
            ...
            <p class="center col-md-1" align="right">
                    <a href="acc_trans_analysis.php" title="Click here to reveal charts " data-toggle="tooltip" class="btn btn-primary btn-sm"><i
                                class="glyphicon glyphicon-chevron"></i> Blank page? Refresh to reveal charts</a><p>
                                <?php
								$sql =$database->query("SELECT SUM(have_paid) as have_paid FROM `acc_school_fees_payments` WHERE `sess_id`= '{$sess_id}' AND `term_id`='{$term_id}' ORDER BY route_id DESC ");
		$sql=$database->fetch_array($sql);
		$count =$database->query("SELECT distinct `student_id` FROM `acc_school_fees_payments` WHERE `sess_id`= '{$sess_id}' AND `term_id`='{$term_id}' ORDER BY student_id DESC ");

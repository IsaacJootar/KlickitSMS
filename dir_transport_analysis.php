...
$session=$database->query("SELECT * FROM `session_manager` WHERE `status`='Current Session'");
	...
	$term_id=$term['id'];
     	$strQuery = "SELECT student_id, expected_to_pay, route_id, sum(have_paid) as amount FROM acc_student_bus WHERE `sess_id`= '{$sess_id}' AND `term_id`='{$term_id}' group by route_id";
;
     	...
     	if ($result) {
        	...
        	$arrData = array(
        	    "chart" => array(
                  "caption" => "Students Bus  analysis chart",
...
// Render pie chart
 $pieChart->render();
	 ...
	 $pareto2d->render();
	 $scrollline2d = new FusionCharts("spline", "scr" , 800, 500, "chart-3", "json", $jsonEncodedData);
    ...
    <div class="box col-md-12">
        ...
        <div class="box-header well" data-original-title="">
            ...
            <p class="center col-md-1" align="right">
                    <a href="dir_transport_analysis.php" title="Click here to reveal charts " data-toggle="tooltip" class="btn btn-primary btn-sm"><i
                                class="glyphicon glyphicon-chevron"></i> Blank page? Refresh to reveal charts</a><p>
                                <?php
								$sql =$database->query("SELECT SUM(have_paid) as have_paid FROM acc_student_bus WHERE `sess_id`= '{$sess_id}' AND `term_id`='{$term_id}' ORDER BY route_id DESC ");
		$sql=$database->fetch_array($sql);
		$count =$database->query("SELECT distinct `student_id` FROM acc_student_bus WHERE `sess_id`= '{$sess_id}' AND `term_id`='{$term_id}' ORDER BY student_id DESC ");
...
?>
 <h3 align=center style="color:#000">Bus fare  total of  <?php echo  format_currency($sql['have_paid'])?> have been paid by <?php echo  $count ?> Student(s)</h3>;

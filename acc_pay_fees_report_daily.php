...
<script src="jquery-ui-1.12.0/jquery-1.12.4.js"></script>
 ...
 <div align="center">
              ...
              <div class="alert alert-info" align="center">
               <h4>Genarate daily income report</h4></div>
                <a href="acc_pay_fees_reports.php" style="float:left" title="Go back to  payment details"  data-toggle="tooltip" class="btn btn-primary btn-sm"><i
                              ...
                              <a class="btn btn-default" title="click here to refreash page " data-toggle="tooltip" href="acc_pay_fees_report_daily.php" style="float:right"><i class="glyphicon glyphicon-refresh"></i>Refresh page</a></br>
...
self.location='acc_pay_fees_report_daily.php?cat=' + val ;
}

</script>
<b><h5>Select a session, term and  corresponding student type to generate report</h5></b><p><p>
<?Php
...
?>                        
             <div class="box col-md-11">
               <div class="box-inner">
                 <div class="box-header well" data-original-title="">
                   <h2>Daily income report generated for [ <?php echo date("Y-m-d");?>]              </h2>
     ...
     <?php
      $date=date("Y-m-d");
if($_POST['type']==0){
    $expense=accountschoolFees::find_by_sql("SELECT * FROM `acc_student_fees_with_items` WHERE `sess_id`='{$_POST['cat']}' AND `term_id`= '{$_POST['subcat']}' AND `date`= '{$date}'  AND `student_type`= 0");
    
}

if($_POST['type']==1){
    $expense=accountschoolFees::find_by_sql("SELECT * FROM `acc_student_fees_with_items` WHERE `sess_id`='{$_POST['cat']}' AND `term_id`= '{$_POST['subcat']}' AND `date`= '{$date}' AND `student_type`= 1");
    
}
if($_POST['type']==2){
    $expense=accountschoolFees::find_by_sql("SELECT * FROM `acc_student_fees_with_items` WHERE `student_type` BETWEEN 0 AND 1 AND `sess_id`='{$_POST['cat']}' AND `term_id`= '{$_POST['subcat']}' AND `date`= '{$date}'");
	   ...
	   ?>        
				 ...
				 echo $total=accountschoolFees::find_total_number_of_payments($sess_id, $term_id);?>
                  <strong style="color:#F00" style="float:inherit">students</strong>
                   ...
                   </table><th width="400"><?php echo "<a target='_blank' class='btn btn-primary btn-sm' href=\"acc_pay_fees_report_daily_print.php?sess_id=".(urlencode($_POST['cat']))."&&term_id=".(urlencode($_POST['subcat']))."&&class=".(urlencode($_POST['class']))."&&type=".(urlencode($_POST['type']))."\"><i class='glyphicon glyphicon-print' ></i>  Print this report</a>" ?>
                      </th>
            </div>

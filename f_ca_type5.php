 ...
 <div align="center">
  ...
  $error_flag=false;
         ...
         ?>
                 ...
                 <form action="f_ca_type4_exe.php" method="post">
                  <table class="table">
                     <thead>
    ...
    while($values=mysql_fetch_array($sql)){ 
      ...
      $qry=mysql_fetch_array($sql2)
                    ?>   
                                      <td class="center"><img src="images/avatar.png" width="25" height="25"/><?php echo $values['fullname']; ?></td>
...
<input type="number" <?php if($database->num_rows($check_status)== 1){echo 'disabled';
                    ...
                    <input type="number" <?php if($database->num_rows($check_status)== 1){echo 'disabled';
    }?> size="8" min="0" max="<?php echo $config['score2'] ?>"step=0.01 step=0.01 title="<?php echo $config['CA2']. 'cannot be more than'.  $config['score2']; ?> " name="<?php echo $values['student_id'].'ca2' ;?>" placeholder="<?php echo  $config['CA2'];?>"    value="<?php echo $qry['CA2'];?>" /> 
                    <input type="number" <?php if($database->num_rows($check_status)== 1){echo 'disabled';
     ...
     <input type="number" <?php if($database->num_rows($check_status)== 1){echo 'disabled';
    }?> size="8" min="0" max="<?php echo $config['score4'] ?>" step=0.01  title="<?php echo $config['CA4']. 'cannot be more than'.  $config['score4']; ?> " name="<?php echo $values['student_id'].'ca4';?>" placeholder="<?php echo  $config['CA4']; ?>" value="<?php echo $qry['CA4'];?>" />
    <input type="number" <?php if($database->num_rows($check_status)== 1){echo 'disabled';
    }?> size="8" min="0" max="<?php echo $config['score5'] ?>" step=0.01  title="<?php echo $config['CA5']. 'cannot be more than'.  $config['score5']; ?> " name="<?php echo $values['student_id'].'ca5';?>" placeholder="<?php echo  $config['CA5']; ?>" value="<?php echo $qry['CA5'];?>" />  

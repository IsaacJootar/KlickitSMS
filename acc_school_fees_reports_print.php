...
?> </h2>
 ...
 <hr>
    ...
    <td width="206"> <?php echo 'Tuition Fees Payment Report (Current Term)';?></td>
...
<?php 
 global $database;
  $other_items=accountschoolFees::find_by_sql("SELECT * FROM `acc_school_fees_payments` 
   ...
   <td>
    <?php foreach ($other_items as $oth) { ?>
  <span class='label label-default'> <?php  echo ucwords($oth->item_name); ?>: </span>

 <?php echo ' '. ' ' . format_currency($oth->have_paid);?>
 <?php } ?>
</td>
...
<p><div align="center"> A total of  <?php echo '<strong>'.$summary=accountschoolFees::find_student_count_by_class_and_type($sess_id, $term_id, $class_name, $s_type).'<strong>'; echo ' '; ?>Student(s)  Paid tuition  fee of <?php echo format_currency($tuition=accountschoolFees::find_total_tuition_fee_per_term($sess_id, $term_id, $class_name, $s_type)) ;?> only this term.
<p>

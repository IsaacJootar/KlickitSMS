...
<div class="box-content row">
  ...
  ?>
<table class="table table-striped table-bordered" border="1" align="center" cellpadding="0" cellspacing="0">
...
?> </h2>
 ...
 <div align="center"><a href="javascript:window.print()">[print or save]</a></div>
    ...
    <td width="206"> <?php echo 'Termly Attendance Sheet';?></td>
...
<p>
   ...
   <tr class="transcriptheader hdsmall">
     ...
     $sql=$database->query("SELECT DISTINCT `week`  FROM `student_attendance`
    WHERE `class_name`='{$class_name}' AND `sess_id`='{$sess_id}'
     AND `term_id` = '{$term_id}'");

    while($weeks=$database->fetch_array($sql)){ ?> 
    <td><div align="left"><strong>Week <?php echo $weeks['week']; ?></strong></br>
      </div></td>
   ...
   <?php  
   $st=$database->query("SELECT `id`, `student_id`, `fullname` FROM `student_class`
     WHERE `class_name`='{$class_name}'
    AND `account_status` = 1 ORDER BY trim(fullname) ASC ");
    while($students=$database->fetch_array($st)) { ?>
    <tr>
 
 
<td><div align="left"><?php echo  $students['fullname']; ?></div></td>
 <?php
 $student_id= $students['student_id'];
    $sql=$database->query("SELECT  *  FROM `student_attendance` WHERE `class_name`='{$class_name}' AND `sess_id`='{$sess_id}' AND `term_id` = '{$term_id}' AND `student_id`= '{$student_id}' GROUP BY `week` ");
while ($term_total=$database->fetch_array($sql)) { 
?>
 
  
 
<td><div align="left"><?php output_total($student_id, $term_id, $class_name, $term_total['week'])?></div> </td>

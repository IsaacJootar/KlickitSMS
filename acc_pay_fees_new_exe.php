...
$check_config=$database->query("SELECT * FROM `acc_configure_tuition_fees` WHERE `class_name`= '{$class_name}' AND `sess_id`='{$sess_id}' AND `term_id`= '{$term_id}' AND `status`=0");
 ...
 foreach($_POST['other_items'] as $id => $value):
	...
	$pay->item_name=$id;
	$sql1=$database->query("SELECT * FROM  `acc_configure_school_fees_items` WHERE `sess_id`='{$sess_id}' AND  `term_id`= '{$term_id}' AND `status`=0  AND `class_name`='{$class_name}' AND `item_name` = '{$pay->item_name}' ");

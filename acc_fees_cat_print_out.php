...
?> 
 ...
 <hr>
  ...
  <tr class="transcriptheader hdsmall">
    <td>CATEGORY TYPE:</td>
    <td><?php  if($cat== 1){ echo "Full Payment";} else {echo "Part Payment";}?></td>
    <td>CLASS:</td>
    <td><?php 
			echo $class;
	 ?></td>
...
<td width="240"><div align="center"><strong>Last Payment Date</strong></div></td>
   ...
   <?php  
 global $database;
 $result=$database->query("SELECT * FROM `acc_student_fees` WHERE `sess_id`='{$sess}' AND `term_id`= '{$term}' AND `class_name`='{$class}' AND `status`= '{$cat}'");

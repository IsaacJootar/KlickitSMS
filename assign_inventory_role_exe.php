...
$qry=$database->query("UPDATE `system_roles` SET `role`='{$role}' WHERE `user_id`= '{$staff_id}'");
	
			
                
			
			$session->message("Staff has been assigned Inventory / store  officer role successfully, please note that multiple assignments will be ignored ");
		redirect_to('manage_roles.php');
		exit();

...
$session=$database->query("SELECT * FROM `session_manager` WHERE `status`='Current Session'");
	...
	$term_id=$term['id'];
     	...
     	$strQuery = "SELECT item_name, sum(expense_amount) as amount FROM `acc_expense` WHERE `sess_id`= '{$sess_id}' AND `term_id`='{$term_id}' group by item_name  ";
     	...
     	if ($result) {
        	...
        	$arrData = array(
        	    "chart" => array(
                  "caption" => "Expense and Item amount analysis chart",
    ...
    <div class="box col-md-12">
        ...
        <div class="box-header well" data-original-title="">
            ...
            <p class="center col-md-1" align="right">
                    <a href="dir_expense_analysis.php" title="Click here to reveal charts " data-toggle="tooltip" class="btn btn-primary btn-sm"><i
                                class="glyphicon glyphicon-chevron"></i> Blank page? Refresh to reveal charts</a> 
                 
                     
                                             <?php
								$sql =$database->query("SELECT SUM(expense_amount) as amount FROM acc_expense WHERE `sess_id`= '{$sess_id}' AND `term_id`='{$term_id}'");
		$sql=$database->fetch_array($sql);
		$count =$database->query("SELECT distinct `item_name` FROM `acc_expense` WHERE `sess_id`= '{$sess_id}' AND `term_id`='{$term_id}' ORDER BY item_name DESC ");
...
?>
 <h3 align=center style="color:#000">A total of  <?php echo  format_currency($sql['amount'])?> has been spent on <?php echo  $count ?> Items</h3>;
